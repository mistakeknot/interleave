# interleave Philosophy

## Purpose
Deterministic Skeleton with LLM Islands — token-efficient document generation via template-then-fill.

## North Star
Advance interleave through small, testable changes aligned to its core mission: Deterministic Skeleton with LLM Islands — token-efficient document generation via template-then-fill.
